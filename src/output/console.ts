...
export class ConsoleExporter {
  ...
  private displayHeader(report: AnalysisReport): void {
    ...
    process.stdout.write(chalk.gray(`Generated: ${report.metadata.generatedAt.toLocaleString()}`) + '\n');
  ...
  private displayTopAuthors(authors: any[]): void {
    ...
    process.stdout.write(table(authorData, { border: { bodyLeft: '║', bodyRight: '║', bodyJoin: '│' } }));
    process.stdout.write('\n');
  }
  ...
  private displayHotspots(files: any[]): void {
    ...
    process.stdout.write(table(fileData, { border: { bodyLeft: '║', bodyRight: '║', bodyJoin: '│' } }));
    process.stdout.write('\n');
  }
  ...
  private displayGovernance(governance: any): void {
    ...
    process.stdout.write(`${chalk.bold('Governance Score:')} ${Math.round(governance.score * 100)}%\n`);
    process.stdout.write('\n');
  ...
  private displayFooter(report: AnalysisReport): void {
    ...
    process.stdout.write(chalk.gray(`Analysis completed in ${report.metadata.processingTime}ms`) + '\n');
    process.stdout.write(chalk.blue('═'.repeat(60)) + '\n');
    process.stdout.write('\n');
  }
