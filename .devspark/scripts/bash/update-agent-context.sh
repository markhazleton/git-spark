...
validate_environment() {
    ...
    if [[ -z "$CURRENT_BRANCH" ]]; then
        ...
        if [[ "$HAS_GIT" == "true" ]]; then
            ...
            log_info "Set DEVSPARK_FEATURE environment variable or create a feature first"
        fi
        exit 1
    fi
    ...
    if [[ ! -f "$NEW_PLAN" ]]; then
        ...
        if [[ "$HAS_GIT" != "true" ]]; then
            log_info "Use: export DEVSPARK_FEATURE=your-feature-name or create a new feature first"
        fi
        exit 1
    fi
